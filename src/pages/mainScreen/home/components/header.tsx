...
const Header = ({ layout, options, onChange }: BottomTabHeaderProps & { onChange: (value: any) => void }) => {
  ...
  return (<Appbar.Header style={{ backgroundColor: 'transparent' }}>
    ...
    <Appbar.Content onPress={handlePresentModalPress} title={(<Text numberOfLines={1} className="text-right  text-base font-semibold  absolute top-[-10]   items-center justify-center right-0">{findIndex(shop.select.id)?.name}</Text>)} tvParallaxTiltAngle={1} />
